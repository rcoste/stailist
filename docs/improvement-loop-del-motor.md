...
4. **La regla pre-registrada, ANTES de generar.** Qué cuenta como ganar y qué se
   ...
   **UN UMBRAL SIN NÚMERO NO ES UN UMBRAL** (lección de 65ded440 y repetida en
   870c625d, así que va aquí en letra grande): "que suba", "más que el
   control", "≥" a secas — todas se cumplen con un delta de 1 sobre 47, que es
   ruido de barajeo. Escribe el número o la proporción que hace falta para
   creerlo, y escríbelo antes de ver el resultado.

   **Y CUANDO EL CAMBIO TIENE UN MECANISMO CONTABLE, PONLE SU GATE ANTES DEL
   VOTO.** Si se puede contar sin juez y sin Roberto —cuántos looks traen la
   prenda, dónde cae el acento (`scripts/cruce-acentos.ts`), cuántas veces
   dispara la regla— ese conteo va PRIMERO: si el mecanismo no se movió, la
   ronda se cierra ahí. En 870c625d eso ahorró los jueces y, sobre todo, sus
   10 minutos de votar algo que no podía decidir nada.
...
| 08-26 | 870c625d | v74 | prompt-anterior (v73) | ❌ CERRADA SIN VOTAR — **el gate de mecanismo la paró antes del voto** | v74 = la camiseta entra al piso de formalidad de evento/noche (no estaba listada entre las piezas que elevan, así que ahí sólo salía de base). **El gate, pre-registrado y corrido ANTES de pedir su voto y antes de pagar jueces: camiseta VISIBLE 4 looks en v74 contra 3 en v73 — delta +1 de 47, y en los briefs sociales 1 contra 0.** Eso es un look; es indistinguible de la varianza del barajeo. Pasa la LETRA de mi gate ("más que el control") y no su espíritu — **el mismo error de redacción que la bitácora ya registró en 65ded440, cometido otra vez: un umbral sin número no es un umbral.** Se cierra sin votar y **v74 NO entra**: meter texto de prompt sin efecto demostrable es acumular superficie, que es la lección de v56 (a diferencia de v73, aquí no hay divergencia laboratorio-producción que justifique el pase). Costo: $6.70 de generación; los jueces NO se pagaron (~$1.60 ahorrados) y su tiempo tampoco. **Lo que el resultado enseña, y vale más que la ronda:** una línea en el piso de formalidad NO es la palanca del repertorio — el arquetipo, el dial y la formalidad empujan más fuerte que una viñeta. Si se quiere de verdad la camiseta en registro alto hay que tocar algo con más peso, y eso es otra vuelta. **Evidencia que SÍ queda (98 looks votados con camiseta): visible bajo capa abierta 21 looks 86%, sola 4 looks 100%, invisible 73 looks 81% — el motor ya lo hace y él lo aprueba MEJOR que la media. Y la camiseta de base NO es defecto: es `sueter-sin-base`, que él pidió, y los votos la respaldan (cuello de tortuga sin nada debajo: 25%, 1👍/3👎).** |
| — | — | — | — | — | 🧵 **CLÓSET DE REFERENCIA v1.1 (2026-08-26) — corte de línea base, corrección de dato.** Roberto, votando: *"es más como charcoal, y es de franela no lana"*. La prenda quedó `Pantalón de vestir de franela carbón · gris carbón #3A3B3F · franela` (era "de lana gris carbón" con hex `#8A8A8A` —gris medio— e imagen prestada del pantalón gris CLARO, que es lo que lo hacía ver claro en pantalla). **Lo que estaba mal era el HEX y la IMAGEN; el nombre decía la verdad** — al revés de lo que parecía al descubrirlo. También se quitó la imagen prestada del "Short de lino azul marino" (mostraba `bermuda-sastre-carbon.png`): no hay arquetipo de short de lino marino de hombre y un swatch honesto es mejor que la foto de otra prenda — al votar, una imagen equivocada hace juzgar la prenda equivocada. **v73 RE-CONGELADO** bajo pool v11 con el clóset corregido. Distinción útil que salió de aquí: corregir `image_path` NO cambia lo que el motor lee (no viaja al prompt) y no obligaría a re-congelar; corregir nombre/color/hex/material SÍ. Dato de contexto: **ningún bottom del clóset tiene `material`** — "de lana" vivía sólo en el nombre; éste es el primero con material real |

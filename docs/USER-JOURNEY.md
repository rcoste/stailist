...
## Estado actual del código (2026-06-23; motor/viaje actualizados 2026-07-01, v21)
...
- ✅ Motor de outfits (`lib/engine/`): prompt **v21** versionado (datos ricos por prenda — material/patrón/color secundario — + borrador de razonamiento interno antes de comprometer outfits), structured outputs con enum de `item_ids` (imposible inventar prendas), juez de 2ª pasada en Claude Sonnet 5 (veredicto ok/reparado/rechazado; `JUDGE_MODEL` compartido con Viaje), clima Open-Meteo con fallback, regla de variedad 14 días, **loop de aprendizaje** (`taste-signal.ts`), streaming NDJSON, votos idempotentes, instrumentación TTV/timing/critic_review

...
| 3 | **Swipes de gustos** | ~15 fotos de looks; ❤️/✕ por gesto O botones (desktop + accesibilidad). Genera el taste vector (tags estéticos → contexto del prompt, sin ML) | 30s | ✅ construido: 22 estilos × 2 géneros con fotos reales en `public/looks/` (44 imágenes) → `computeTasteTags` saca 8 tags |
...
| 8 | **Clóset crece a su ritmo** | Fotos opcionales → IA detecta atributos → confirmación editable en un tap. Falla la detección → captura manual, nunca bloquea | ✅ construido: upload en `/closet` (`add-photo-flow.tsx`) → `/api/analizar-prenda` (Claude Opus visión extrae 7+ atributos) → confirmación editable antes de guardar |
...
## Estado actual del código (2026-06-23)
...
- ✅ Motor de outfits (`lib/engine/`): prompt **v16** versionado, structured outputs con enum de `item_ids` (imposible inventar prendas), juez de 2ª pasada (veredicto ok/reparado/rechazado), clima Open-Meteo con fallback, regla de variedad 14 días, **loop de aprendizaje** (`taste-signal.ts`), streaming NDJSON, votos idempotentes, instrumentación TTV/timing/critic_review
...
- ✅ Viaje / itinerario: wizard multidestino → cápsula + clima histórico por parada + outfits del clóset (`/api/trip`)
...
- ✅ Vetos de estilo (hard NOs), silueta (señal suave), "el toque" (tip de styling del juez)

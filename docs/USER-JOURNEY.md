...
| 3b | **¿Cómo nos conociste?** | Un toque, se puede saltar ("prefiero no decir"). Sólo cuentas nuevas, tras guardar la edad. Complementa `profiles.origen` (etiquetas del link) con lo que el link no ve; ambas en `/admin/adquisicion` (v0.2.330.0) | `app/onboarding/conocio/` |
...
| 7 | **Clóset exprés** | Primero una pantalla "tu clóset, sin pasar horas" (foto → varias prendas, v0.2.329.0; no es step numerado). Luego el checklist de básicos con imagen de arquetipo: **57 para mujer** (49 + 8 unisex), **53 para hombre** (45 + 8). Pestaña **Trajes** = las dos piezas en una tarjeta (`attrs.conjunto`); quien sólo tiene el saco lo marca en Sacos. CERO fotos obligatorias | `app/onboarding/closet/` |
...
| 10b | **Arma tu semana** | `/semana`: un look planeado por día para los 7 días desde mañana (L-V marcados), "día a día" o "trabajo". Mínimo 10 prendas. Se arman en fila en background con el núcleo del look planeado; la fila se retoma sola si se corta. Cada día amanece como look de hoy vía `promoverPlaneado`. El correo del lunes lleva aquí (v0.2.331.0) | `app/semana/`, `app/api/semana/`, `lib/semana.ts` |

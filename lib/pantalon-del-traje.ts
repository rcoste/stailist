...
export function nombrePantalon(nombreSaco: string): string {
  ...
  const limpio = nombreSaco
    .trim()
    .replace(/^saco(\s+de\s+(traje|vestir))?\b\s*/i, "")
    .replace(/^(blazer|traje)\b\s*/i, "")
    .trim();
  // Sin nada que describir, un nombre genérico es mejor que "Pantalón de traje ".
  return limpio ? `Pantalón de traje ${limpio}` : "Pantalón de traje";
}

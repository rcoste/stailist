...
- [ ] **Backfill de atributos ricos (material / patrón / color secundario)**: desde v21 el análisis de visión extrae `material`, `patron` y `color_secundario` y los motores los usan (estampados que pelean, lana en calor, color real de bicolores) — pero solo las prendas agregadas DESPUÉS del cambio los traen. Backfill = re-analizar en lote las prendas existentes con foto/render (`items` con `photo_path` o `render_path`) para llenar SOLO los campos nuevos (no tocar nombre/categoría/hex ya confirmados por el usuario). Las de arquetipo pueden derivarse del catálogo (nombre + descripción) sin visión. **Usar un modelo barato, no Opus**: es extracción simple de 3 campos — `claude-haiku-4-5` como primera opción (validar contra Opus en ~10 prendas de muestra; si el material sale flojo, subir a Sonnet solo ese campo). *Por qué ahora no*: los campos son opcionales y los motores degradan bien sin ellos; con cero usuarios el clóset afectado es chico. *Trigger*: antes del experimento con Tatiana/Toño (para que el motor juzgue su ropa con señal completa), o cuando un clóset real pase de ~20 prendas sin atributos ricos. *Nota del red team (2026-07-01)*: al hacerlo, extender también `updateItemAttrs` + la UI de edición para poder corregir material/patron/color_secundario — hoy son write-once, y una mala lectura de visión ("lana" en un suéter de algodón) excluye la prenda de looks de calor sin que el usuario pueda arreglarlo; un backfill masivo con modelo barato multiplica ese riesgo.

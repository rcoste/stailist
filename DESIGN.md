...
- **App clara, try-on oscuro:** toda la navegación es en claro; el único momento oscuro **de la app** es el try-on ("verte con el look puesto") — fondo casi-negro `#0a0a0a`, el "ta-da" inmersivo. **La landing es la excepción, desde 2026-09-08:** tres secciones enteras en ese mismo negro (ver "Tema por ámbito").
...
- **Tema por ámbito — `.tema-oscuro` / `.tema-claro` (globals.css, 2026-09-08):** un contenedor con `.tema-oscuro` redefine los tokens `--c-*` para todo lo que cuelga de él; los hijos siguen pidiendo `var(--c-ink)`, `bg-surface`, `border-line`… y no se enteran de que están sobre negro. El fondo es el MISMO `#0a0a0a` del try-on (no es un negro nuevo: es el que ya existía escrito a mano). Texto y hairlines van en blanco translúcido (78% / 60% / 42% / 16% / 10%); el acento se invierte, así que un botón "de tinta" sale blanco. `.tema-claro` es la vuelta —una isla clara dentro de una oscura— y hace falta de verdad: **las fotos de prenda traen su fondo crema quemado y se integran con `mix-blend-mode: multiply`; sobre negro, multiply las apaga por completo.** Las dos clases declaran SÓLO tokens, nunca `background` ni `color`: pintar desde ahí le ganaría en silencio a las utilidades de Tailwind. **NO es dark mode** (no responde a la preferencia del sistema): es un ámbito que se pide a mano, sección por sección.
...
- **Dark mode:** DIFERIDO post-MVP (decisión registrada). Sigue sin existir: `.tema-oscuro` es un ámbito que se pide a mano, no una preferencia del sistema.
...
| 2026-08-11 | **Coach-mark = tarjeta blanca sobre velo al 38%; nuevo token `--shadow-float`** (única sombra fuerte del sistema, solo para lo que flota sobre un scrim). Antes el tip era una burbuja negra. | Handoff del rediseño de Inicio. El negro del tip era exactamente el de los botones (CTA, botón central, fit check), así que el tip se leía como algo que tocar. Blanco + velo lo separa; la sombra es lo único que lo despega del fondo apagado. Prohibida en cards, hojas y botones del flujo normal |
| **2026-09-08** | **Tres secciones de la landing en negro** (el reframe "decidir agota", el try-on y el cierre) + **tema por ámbito** con `.tema-oscuro` / `.tema-claro` y tokens fuente `--claro-*` / `--oscuro-*`. | Roberto: "todo es del mismo tono, te pierdes cuando empieza una sección u otra". Cierto y con causa: las 11 secciones eran idénticas —mismo papel, mismo padding, una hairline de 1px como única separación— y el 2026-09-07 se borró el bloque de objeción, que era el único respiro oscuro que quedaba. Se eligieron por SIGNIFICADO, no por alternancia: el try-on porque en la app ese momento ya ES la superficie oscura; el cierre porque cerrar en negro marca el final; el reframe porque es la sección más corta y en negro se lee como manifiesto. Descartadas las de texto largo con listas ("para quién es"), que en negro pesan. |
...
| **2026-09-10** | **Preguntas frecuentes en la landing (sección 10; el cierre pasa a 11)**: `<details>` nativo con el lenguaje de `.biglist` — hairlines, sin tarjetas, pregunta en tinta, respuesta en ink2, signo +/– en la serif itálica apagada de los numerales. Sin animación nueva. | Roberto aprobó las preguntas para que buscadores y asistentes de IA (ChatGPT, Claude) sepan qué es stailist. `<details>` porque abre sin JavaScript y las respuestas quedan en el HTML aunque estén cerradas. En claro, no en negro: es texto largo con lista, lo mismo que se descartó para "para quién es". Texto en `lib/ficha-publica.ts`, compartido con el JSON-LD y `/llms.txt`. |

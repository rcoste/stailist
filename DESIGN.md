...
- **Patrón "héroe 2-col" (F3):** pantallas core en `lg` = página de revista, no scroll de teléfono. Columna visual sticky a la izquierda (`lg:sticky lg:top-20`), texto + acciones a la derecha. Aplicado a: **Hoy** (card sticky izq / título + justificación + acciones der — grid con placement explícito para no alterar el orden del DOM móvil; la justificación se oculta en la card con `rationaleClassName="lg:hidden"` y se reimprime a la derecha), **Viaje·detalle** (resumen/clima izq / maleta+looks der, `lg:flex-row`), **Cápsula** (por qué es tuya + avisos izq / lista de prendas der, `lg:flex-row`).
